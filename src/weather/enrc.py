...
def find_csvfile(name):
    return str(rootdir) + f"/data/weather/{name}"
...
dtype = {"Date/Time": to_localhour, "Temp (°C)": to_float, "Rel Hum (%)": to_float}
columns = {
    "Date/Time": "localtime",
    "Temp (°C)": "temperature",
    "Rel Hum (%)": "humidity",
...
def download_all():
    for key, value in station_list.items():
            ...
            for station, years in value.items():
                ...
                for year in range(years[0], years[1] + 1):
                    ...
                    for month in range(1, 13):
                        ...
                        c.setopt(
                            c.URL,
                            f"https://climate.weather.gc.ca/climate_data/bulk_data_e.html?format=csv&stationID={station}&Year={year}&Month={month}&Day=1&timeframe=1&submit=Download+Data",
                        )
                        ...
                        if c.getinfo(pycurl.HTTP_CODE) != 200:
                            ...
                            df = pd.read_csv(
                                StringIO(body), usecols=columns.keys(), converters=dtype
                            )
                            df = df.filter(list(columns.keys()))
                            df = df.rename(mapper=columns, axis="columns")
                            ...
                            if len(df) > 0:
                                for field, value in process.items():
                                    if callable(value[2]):
                                        value[2] = value[2]()
                                    df[field] = df.apply(
                                        lambda row: value[0](row, value[1], value[2]),
                                        axis=1,
                                    )
                            blocks.append(df)
                        sys.stdout.flush()
        ...
        print(
            f"{saveas}: {first} to {last}, {count} records, {hours-count:.0f} hours missing, {count/hours*100.0:.1f}% coverage"
        )
        sys.stdout.flush()
