...
class config:
    ...
    eu_dict_elec_ceus = dict(
        zip(
            ["Heat", "Cool", "HotWater", "Cook"],
            ["other_electric_heat", "cooling", "water_heating", "cooking"],
        )
    )
    res_comp_dict_ceus = dict(
        zip(
            [
                "Heat",
                "Cool",
    ...
    weather_enduses_rbsa = {
        "sensitive": ["cooling", "heating", "other"],
        "insensitive": [
            "computer",
            "dryer",
            "entertainment",
    ...
    region_df = pd.DataFrame(
        {
            "state": ["ID", "MT", "OR", "WA"],
            "region": ["Mountain", "Mountain", "Pacific", "Pacific"],
        }
    )
...
def read_files():
    siteid_df_dict = {}
    for csv in os.listdir(path_adder(path, setup_config.rbsa_sites_folder)):
        if csv.endswith(".csv"):
            siteid_df_dict[csv.split(".")[0]] = pd.read_csv(
                path_adder(path, setup_config.rbsa_sites_folder + csv),
                converters={"time": get_datetime},
            )
...
def weather_season(location, day, aws, showplots=False):
    ...
    if aws == True:
        ...
        try:
            loc = pd.read_csv(
                path_adder(path, setup_config.noaa_folder % (location)),
                converters={"localtime": noaa_datetime},
            )
        except:
    ...
    time = [
        ...
        for t in loc["localtime"]
    ]
    loc["localtime"] = time
    loc["hr"] = [t.hour for t in loc["localtime"]]
    if weekday:
        loc["weekday"] = np.where(
            np.array([t.weekday() for t in loc["localtime"]]) < 5, 1, 0
        )
    ...
    loc["date"] = [
        datetime.datetime(t.year, t.month, t.day, 0, 0, 0) for t in loc["localtime"]
    ]
    ...
    loc_daily_max = loc_daily_max[
        loc_daily_max["heatindex"].between(thigh - 0.5, thigh + 0.5)
    ]
    ...
    loc_day = (
        pd.merge(
            loc,
            pd.DataFrame({"date": loc_daily_max.index.values}),
            on="date",
            how="inner",
        )
        .groupby("hr")
    ...
    loc_daily_min = loc_daily_min[
        loc_daily_min["heatindex"].between(tlow - 0.5, tlow + 0.5)
    ]
    ...
    loc_day = (
        pd.merge(
            loc,
            pd.DataFrame({"date": loc_daily_min.index.values}),
            on="date",
            how="inner",
        )
        .groupby("hr")
    ...
    spring_temp = np.array(
        loc[loc["month"].between(4, 6)].groupby("hr").mean()["heatindex"]
    )
    ...
    for col in sensitivities.keys():
        ...
        for h in range(24):
            ...
            try:
                q.append(
                    sn.get_load(
                        model, h, spring_temp[h], normalize=False, convert=list
                    )[0]
                )
            except:
                q.append(
                    sn.get_load(model, h, spring_temp[h], normalize=False, convert=None)
                )
            try:
                r.append(
                    sn.get_load(
        ...
        if showplots:
            ...
            plt.savefig(
                path_adder(
                    path, f"{location}/{location}_loadshape_{btype}_{col}_{day}.png"
                )
            )
            plt.close()
...
def comp_enduses(
    ...
    for i in set(electrification["region"]):
        ...
        for j in ["commercial", "residential"]:
            x[j] = electrification[
                (electrification["region"] == i)
                & (electrification["building_type"] == j)
            ].iloc[0][3:]
    ...
    for btype in sens[0].keys():
        ...
        res_comp = components_df[
            components_df["building_type"] == get_keys(config.btype_dict, btype)
        ].dropna(axis=1)
        ...
        try:
            area, bcount = feeder_comp[
                (feeder_comp["feeder_type"] == feeder)
                & (feeder_comp["building_type"] == inverse_mapping(build_map)[btype])
            ].iloc[0][3:]
        except:
            area = 0
            bcount = 0
        ...
        for eu in sensitivities.keys():
            ...
            try:
                elec = loc[loc["building_type"] == "commercial"][
                    config.eu_dict_elec_ceus[eu]
                ][0]
            except:
                elec = 1
            # if eu == 'Cool':
    ...
    for btype in sens[0].keys():
        ...
        for eu in sensitivities.keys():
            ...
            try:
                elec = loc[loc["building_type"] == "residential"][
                    inverse_mapping(config.eu_dict_elec)[eu]
                ][0]
            except:
                elec = 1
            # if eu == 'cooling':
            ...
            if hc_ind < cooling_ind:
                if eu == "hc":
                    add_load = [
                        np.where(load[0] > 0, load[0], 0) * elec,
                        np.where(load[0] > 0, load[1], 0) * elec,
                        np.where(load[0] > 0, load[2], 0) * elec,
                    ]
                elif eu == "cooling":
                    winter.append(
                        np.where(load[0] > 0, load[0], 0) * elec + add_load[0]
                    )
                    spring.append(
                        np.where(load[1] > 0, load[1], 0) * elec + add_load[1]
                    )
                    summer.append(
                        np.where(load[2] > 0, load[2], 0) * elec + add_load[2]
                    )
                else:
                    winter.append(np.where(load[0] > 0, load[0], 0) * elec)
                    spring.append(np.where(load[1] > 0, load[1], 0) * elec)
                ...
                if list(sensitivities.keys()).index(eu) == max(indexes):
                    winter[min(indexes)] = (
                        winter[min(indexes)] + np.where(load[0] > 0, load[0], 0) * elec
                    )
                    spring[min(indexes)] = (
                        spring[min(indexes)] + np.where(load[1] > 0, load[1], 0) * elec
                    )
                    summer[min(indexes)] = (
                        summer[min(indexes)] + np.where(load[2] > 0, load[2], 0) * elec
                    )
                else:
                    winter.append(np.where(load[0] > 0, load[0], 0) * elec)
                    spring.append(np.where(load[1] > 0, load[1], 0) * elec)
    ...
    season_dict = {
        "winter": winter_build_load,
        "spring": spring_build_load,
        "summer": summer_build_load,
    }
    for season in ["winter", "spring", "summer"]:
        ...
        df.to_csv(
            path_adder(
                path,
                "%s/%s/%s/%s_%s_%s.csv"
                % (location, season, feeder, location, season, feeder),
            ),
            index=False,
        )
        ...
        # plt.show()
        ax[0].stackplot(
            np.arange(24), season_dict[season] / sum(season_dict[season]), labels=labels
        )
        ax[0].legend(loc="upper left")
        ax[0].set_title(
            "Normalized Hourly composite %s enduse load for %s" % (feeder, location)
        )
...
if __name__ == "__main__":
    ...
    config_settings = pd.read_csv(
        os.path.join(path, pathlib.Path("autotest/config.csv")), header=None
    ).fillna("")
    config_settings = config_settings.rename(columns={0: "Category", 1: "Values"})
    outputs = np.array(
        config_settings[config_settings["Category"] == "City"]["Values"]
    )[0].split()
    seasons = np.array(
        config_settings[config_settings["Category"] == "Season"]["Values"]
    )[0].split()
    ftype = np.array(
        config_settings[config_settings["Category"] == "Feeder"]["Values"]
    )[0].split()
    debug = np.array(
        config_settings[config_settings["Category"] == "Intermediate Results"]["Values"]
    )[0].split()
    ...
    for city in outputs:
        ...
        if len(seasons) == 0:
            print(
                "You have not selected a season. Please input one or more of the following options: Summer, Spring, Winter"
            )
